...
namespace Remote_server_432_project
{
    public partial class Form1 : Form
    {
        ...
        bool server2_keys_recieved = false;
        ...
        private void Receive()
        {
            ...
            while (!terminating && connected)
            {
                try
                {
                    ...
                    if (message.Contains("I_am_a_server"))
                    {
                        ...
                        else
                        {
                            ...
                            server_2_connected = true; 

                            server_2_connection_ack = true;
                            server_2_socket = s;

                            // sending server-2 to server-1 is connected
                            if (socketList.Contains(server_1_socket)) // (connectionTypeList.Contains("server_1")
                            {
                    ...
                    if (server_1_connection_ack && is_send_2)
                    {
                        if (socketList.Contains(server_2_socket)) //connectionTypeList.Contains("server_2")
                        {
                            string msg = "server1_master_server_connected";
                            byte[] msg_b = Encoding.Default.GetBytes(msg);
                            server_2_socket.Send(msg_b);
                    ...
                    if (server_2_connection_ack && is_send_1)
                    {
                        if (socketList.Contains(server_1_socket)) //connectionTypeList.Contains("server_1")
                        {
                            string msg = "server2_master_server_connected";
                ...
                {
                    logs_remote_server.AppendText("YYYYYYYYYYYYYYYYYYYYYYYYYYYYYY" + "\n");

                    logs_remote_server.AppendText(e.Message + "\n");


                    if (s == server_1_socket)  //connectionTypeList[socketList.Count - 1] == "server_1"
                    {
                    ...
                    else if (s == server_2_socket) // connectionTypeList[socketList.Count - 1] == "server_2"
                    {
        ...
        private void fileReplication()
        {
            while (listening)
            {
                ...
                while (not_replicated_files.Count > 0 && not_replicated_file_names.Count > 0 && server_1_connected && server_2_connected && server_1_server_2_connected && !is_send_1 && !is_send_2)
                {
                    ...
                    {
                        logs_remote_server.AppendText("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" + "\n");
...
//if (server_1_connected && server_2_connected && server_1_server_2_connected && server1_keys_recieved && server2_keys_recieved)
//{
//
//        while (not_replicated_files.Count > 0 && not_replicated_file_names.Count > 0 && server_1_connected && server_2_connected && server_1_server_2_connected)
//        {
//
//            byte[] file_data = not_replicated_files[0];
//            string file_name = not_replicated_file_names[0];
